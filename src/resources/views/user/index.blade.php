    ...
    <x-slot name="header">
        ...
        <form method="get" action="{{ route('user.items.index') }}">
            <div class="lg:flex lg:justify-around">
                            ...
                            <optgroup label="{{ $category->name }}">
                                @foreach ($category->secondary as $secondary)
                                    <option value="{{ $secondary->id }}"
                                        @if ((int) \Request::get('category') === $secondary->id) selected @endif>
                                        {{ $secondary->name }}
                                    </option>
                                @endforeach
                        @endforeach
                ...
                <div class="flex">
                    <div>
                        <span class="text-sm">表示順<br></span>
                    ...
                    <div>
                        <span class="text-sm">表示件数<br></span>
                                    ...
                                    <div class="border rounded-md p-2 md:p-4">
                                        ...
                                        <div class="mt-4">
                                            ...
                                            <p class="mt-1 text-sm text-gray-700">
                                                {{ number_format($product->price) }}<span>円(税込)</span>
                                            </p>
                                        </div>
                                    </div>
                                </a>
